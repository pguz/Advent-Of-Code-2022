...
def _sum_items_priorities(items):
    ...
    sum_priorities_of_uppercase_common_items = (items[(items >= 'A') & (items <= 'Z')].map(ord) - offset_uppercase_items).sum()
    sum_priorities_of_lowercase_common_items = (items[(items >= 'a') & (items <= 'z')].map(ord) - offset_lowercase_items).sum()
    return sum_priorities_of_uppercase_common_items + sum_priorities_of_lowercase_common_items
