...
def parse_file(fd):
    ...
    cpu_schedule = instructs_schedule.combine_first(pd.Series(np.zeros(instructs_effective_cycles.iloc[-1])))
    cpu_schedule[0] = 1
    return cpu_schedule,

def sum_of_signal_strengths(cpu_schedule):
    signal_strength_cycles = cpu_schedule.cumsum().shift(1)[cpu_schedule.index % 40 == 20]
    sum_signal_strengths = signal_strength_cycles.reset_index().apply(lambda r: r['index'] * r['value'], axis=1).sum()
    return sum_signal_strengths

def print_crt(cpu_schedule):
    def set_pixels(cycles):
        return ((cycles['index'] % 40) - cycles['value']).abs() <= 1
    crt = cpu_schedule.cumsum().reset_index().pipe(set_pixels).map({True: '#', False: '.'})
