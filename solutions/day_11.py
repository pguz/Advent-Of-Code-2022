...
def parse_file(fd):
    input_template = (
        'Monkey (?P<monkey_id>\d+):\n'
        '  Starting items: (?P<items>(\d+,\s)*\d+)\n'
        '  Operation: new = old (?P<amp_op>\+|\*) (?P<amp_val>\d+|old)\n'
        '  Test: divisible by (?P<div_val>\d+)\n'
        '    If true: throw to monkey (?P<pass_id_true>\d+)\n'
        '    If false: throw to monkey (?P<pass_id_false>\d+)'
    )
    re_pattern = re.compile(input_template, re.MULTILINE)
    monkeys = [create_monkey(re_match.groupdict()) for re_match in re_pattern.finditer(fd.read())]
    return monkeys,
