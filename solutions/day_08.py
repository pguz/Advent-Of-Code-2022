...
def count_visibility(tree_map):
    ...
    fourth_conf_visibility = (
        tree_map.transpose()[::-1].pipe(find_visibility)[::-1].transpose()
    )
    visibility = (
        first_conf_visibility
        | second_conf_visibility
        | third_conf_visibility
        | fourth_conf_visibility
    )
...
def find_highest_scenic_score(tree_map):
    ...
    fourth_conf_scores = (
        tree_map.transpose()[::-1].apply(calc_scenic_scores)[::-1].transpose()
    )
    scenic_scores = (
        first_conf_scores * second_conf_scores * third_conf_scores * fourth_conf_scores
    )
